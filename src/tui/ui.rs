use crate::tui::app::{App, Mode, NewCardForm, VarForm};
...
fn draw_search(f: &mut Frame, app: &App, area: Rect) {
    let count = format!(" {}/{} cards ", app.filtered.len(), app.cards.len());
...
fn draw_status(f: &mut Frame, app: &App, area: Rect) {
    let (text, style) = match &app.status {
        ...
        None => (
            "↵ copy   ^n new   ^e edit   ^d delete   esc quit".to_string(),
